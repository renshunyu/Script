...
svnurl=http://10.1.198.30/svn/UAPPROGRAM/products/ailkiap30/release/$2/realtime_standardize_server
...
mkdir -p ~/patch/$2/安装包/realtime_standardize_server/xxxxx_yyyymmddhhmi/全量/
result=$?
if [ $result != '0' ]
then
  echo "创建目录错误"
...
cp ./target/*tar.gz ~/patch/$2/安装包/realtime_standardize_server/xxxxx_yyyymmddhhmi/全量/
result=$?
if [ $result != '0' ]
then
  echo "复制安装包错误"
